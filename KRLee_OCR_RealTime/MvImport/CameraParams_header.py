...
_MV_NETTRANS_INFO_._fields_ = [
    ('nReceiveDataSize', int64_t),          # < \~chinese 已接收数据大小  [统计StartGrabbing and StopGrabbing之间的数据量]         \~english Received Data Size  [Calculate the Data Size between StartGrabbing and StopGrabbing]  
...
_MV_SAVE_IMG_TO_FILE_PARAM_._fields_ = [
    ...
    # < \~chinese     ch:Bayer格式转为RGB24의插值方法 0-最近邻 1-双线性 2-Hamilton
...
_MV_CC_RECORD_PARAM_T_._fields_ = [
    ...
    ('nWidth', c_ushort),                       # < \~chinese 图像宽(指定目标参数时需为2의倍数)                   \~english Ip config option
    ('nHeight', c_ushort),                      # < \~chinese 图像高(指定目标参数时需为2의倍数)                    \~english Ip config option
...
_MV_CC_FILE_ACCESS_PROGRESS_T._fields_ = [
    ('nCompleted', int64_t),     # < \~chinese 已完成의长度         \~english Completed Length
...
_MV_ACTION_CMD_INFO_T._fields_ = [
    ...
    ('nTimeOut', c_uint),          # < \~chinese 等待ACK의超时时间，如果为0表示不需要ACK               \~english Timeout
