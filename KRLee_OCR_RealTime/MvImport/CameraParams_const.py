...
# \~chinese 设备类型 definition    \~english Device Type Definition
...
MV_MAX_TLS_NUM                               = 8           # < \~chinese 最소支持의 전송 계층 인스턴스 수     \~english The maximum number of supported transport layer instances
MV_MAX_DEVICE_NUM                            = 256         # < \~chinese 최대 지원 장치 수            \~english The maximum number of supported devices

MV_MAX_GENTL_IF_NUM                          = 256         # < \~chinese 최대 지원 GenTL 수           \~english The maximum number of GenTL supported
MV_MAX_GENTL_DEV_NUM                         = 256         # < \~chinese 최대 지원 GenTL 장치 수       \~english The maximum number of GenTL devices supported
...
MV_ACCESS_Control                            = 3
# \~chinese 可以从5의模式下抢占权限，然后以控制权限打开               \~english You can seize the authority from the 5 mode, and then open with control authority
...
# \~chinese 可以从5의模式下抢占权限，然后以可被抢占의控制权限打开     \~english You can seize the authority from the 5 mode, and then open with seized control authority
MV_ACCESS_Monitor                            = 7
